...
import { ListBillingChargesRequest$outboundSchema, ListBillingChargesResponseBody$inboundSchema, } from "../models/listbillingchargesop.js";
 ...
 */
export function billingListBillingCharges(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => ListBillingChargesRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    }), { ctype: "application/jsonl" }), M.fail([400, 401, 403, 404, "4XX"]), M.fail([500, 503, "5XX"]))(response, req);
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=billingListBillingCharges.js.map